...
<body>
	<t:wrapper>
		...
		<div id="secretsTable">
			<h2 class="subHead">Most Popular Secrets...</h2>
			<table>
			<tbody>
			<c:forEach items="${popular}" var="secret">
				<tr>
				...
				<c:choose>
					<c:when test="${user.id == secret.user.id}">
						<td><a class="tLink" href="/secrets/popular/${secret.id}/delete">Delete</a> | </td>
						<c:choose>
						<c:when test="${secret.usersWhoLike.contains(user)}">
						<td><a class="tLink" href="/secrets/popular/${secret.id}/unLike">UnLike</a></td>
						</c:when>
						<c:otherwise>
						<td><a class="tLink" href="/secrets/popular/${secret.id}/like">Like</a></td>
						</c:otherwise>
					</c:choose>
					</c:when>	
					<c:otherwise>
